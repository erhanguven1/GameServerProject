	...
	public class APRController : MonoBehaviour
	{
		...
		void PlayerMovement()
		{
			...
			else
			{
				...
				{
					var v3 = APR_Parts[0].GetComponent<Rigidbody>().transform.forward * (forwardBackwardValue * moveSpeed);
				...
				else if(forwardBackwardValue < 0)
				{
		...
		void PlayerRotation()
		{
			...
			{   
				...
				if (rightleftValue != 0)
				{
					APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation = Quaternion.Lerp(APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation, new Quaternion(APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.x,APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.y - (rightleftValue * turnSpeed), APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.z, APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation.w), 6 * Time.fixedDeltaTime);
				}
		...
		void PlayerReach()
		{
			float mouseYValue = GetComponentInParent<Player>().mouseYValue;

			//Body Bending
			if(1==1)
			{
				if(MouseYAxisBody <= 0.1f && MouseYAxisBody >= -0.1f)
				{
					MouseYAxisBody = MouseYAxisBody + (mouseYValue / reachSensitivity);
				}
			...
			float reachLeftValue = GetComponentInParent<Player>().leftClick ? 1 : 0;
			//Reach Left
			if(reachLeftValue != 0 && !punchingLeft)
			{
				...
				{
					MouseYAxisArms = MouseYAxisArms + (mouseYValue / reachSensitivity);
				}
			...
			float reachRightValue = GetComponentInParent<Player>().rightClick ? 1 : 0;

			//Reach Right
			if (reachRightValue != 0 && !punchingRight)
			{
		...
		void PlayerPunch()
		{
			bool punchRightValue = GetComponentInParent<Player>().e;
			...
			bool punchLeftValue = GetComponentInParent<Player>().q;
